...
def update_DATA( action ):
	...
	list_alley = [ ( 3, x, y ) for ( x, y ) in allies if y >= player[0][1] ]
	list_fireball = [ ( 4, x, y ) for ( x, y ) in fireballGroup if y >= player[0][1] ]
	list_ladder = [ ( 5, x, y ) for ( x, y ) in ladders if y >= player[0][1] ]
	
	
	if len( list_alley ) > 0 :
		pass
	else :
		list_alley.append( [ 3, -999, -999 ] )
	...
	if len( list_ladder ) > 2 :
		pass
	else :
		list_ladder.append( [ 5, -999, -999 ] )
	...
	coff_13 = list_ladder[0][2]
	coff_14 = list_ladder[1][1]
	coff_15 = list_ladder[1][2]
	coff_16 = list_monster[0][2]
	coff_17 = list_monster[0][2]
	coff_18 = list_monster[1][2]
	coff_19 = list_monster[1][2]
	
	print( "steps: " + str( steps ).zfill(6) + " action: " + str(int(action)).zfill(1) + " coff_0: " + str(int(coff_0)).zfill(6) + " coff_1: " + str(int(coff_1)).zfill(6) + " coff_2: " 
			+ str(int(coff_2)).zfill(6) + " coff_3: " + str(int(coff_3)).zfill(6) + " coff_4: " + str(int(coff_4)).zfill(6) + " coff_5: " + str(int(coff_5)).zfill(6)
	)
	
	DATA_row = tf.constant([ contrl, coff_0, coff_1, coff_2, coff_3, coff_4, coff_5, coff_6, coff_7, coff_8, coff_9, coff_10, coff_11, coff_12, coff_13, coff_14, coff_15, coff_16, coff_17, coff_18, coff_19, 
			1, 1, 1, 1, 1, 1, 1, 1, 1 ], shape=(1, 1, 1, 30), dtype=tf.float32)
